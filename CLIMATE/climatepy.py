...
To proceed please enter a country  
...
try:
    ...
    for country in countries:
        ...
        fossil_mult.append((fossil_df.iloc[country_list.index(country), min_year_index:max_year_index+1]).tolist())
        co2_mult.append((co2_df.iloc[country_list.index(country), min_year_index:max_year_index+1]).tolist())
        renewables_mult.append((renewables_df.iloc[country_list.index(country), min_year_index:max_year_index+1]).tolist())
    energysum=[]
    ...
    energy_graph.update_layout(
        title="Energy Consumption (as kg of oil equivalent) Per Capita",
        xaxis_title="period",
    ...
    &nbsp;
    '''
    fossil_graph=px.line(x=years_range, y=fossil_mult)
    fossil_graph.update_layout(
        title="Percentage Of Energy Derived From Fossil Fuels",
        xaxis_title="period",
        yaxis_title="%",
    )
    ...
    &nbsp;
    '''
    renewables_graph=px.line(x=years_range, y=renewables_mult)
    renewables_graph.update_layout(
        title="Percentage Of Energy Derived From Renewable Sources In" ,
        xaxis_title="period",
        yaxis_title="%"
    )
    ...
    &nbsp;
    '''
    co2_graph=px.line(x=years_range, y=co2_mult)
    co2_graph.update_layout(
        title="CO2 Emissions (in Kt) In " + str(countries[0:-1]),
        xaxis_title="period",
